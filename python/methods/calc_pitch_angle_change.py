...
def calc_pitch_angle_change(inp_pwr, fieldlines, params):
    ...
    L_low = params['L_low']
    L_hi  = params['L_hi']
    PWR_THRESHOLD = params['PWR_THRESHOLD']
    ...
    da_N = np.zeros([len(Lshells), len(E_tot_arr), len(tvec) +1])
    da_S = np.zeros([len(Lshells), len(E_tot_arr), len(tvec) +1])
                ...
                for row in fl['crossings'][lat_ind]:
                    ...
                    if damping > 0.1:
                        ...
                        for mres in np.arange(-SCATTERING_RES_MODES, SCATTERING_RES_MODES+1, 1):
                            ...
                            evec_inds = np.arange(0, NUM_E) # do 'em all
